...
def get_seaker_submitted_jobs():
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                "select augury_job_id from seakerUI_job where status = 'Submitted' OR status = 'In Progress'")
...
def set_seaker_job_status(augury_job_id, status):
    try:
        with connection.cursor() as cursor:
            cursor.execute('update seakerUI_job set `status`=%s where `augury_job_id`=%s', (status, augury_job_id))
            connection.commit()
    except pymysql.Error as e:
...
def set_seaker_job_augury_job_id(job_id, augury_job_id):
    try:
        with connection.cursor() as cursor:
            cursor.execute('update seakerUI_job set `augury_job_id`=%s where `id`=%s', (augury_job_id, job_id))
            connection.commit()
    except pymysql.Error as e:
...
def create_augury_job(new_job):
    ...
    set_seaker_job_augury_job_id(job_id, augury_job_id)
    return
